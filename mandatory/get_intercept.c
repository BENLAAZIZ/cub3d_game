...
/*   Updated: 2024/12/01 19:09:06 by hben-laz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_vertical_ray_steps(t_data *data, t_ray *ray, t_var *var)
{
	...
	if (ray->lookingright)
		var->xintercept += T_S;
	var->yintercept = data->p_y + (var->xintercept - data->p_x)
		* tan(ray->rayangle);
	var->xstep = T_S;
...
double	get_v_intercept(t_data *data, t_ray *ray)
{
		...
		&& var.yintercept >= 0 && var.yintercept <= data->height * T_S)
	{
		...
		if (is_wall(data, var.ytocheck, var.xtocheck))
		{
			...
			var.distance = sqrt(pow(data->p_x - var.xtocheck, 2)
					+ pow(data->p_y - var.ytocheck, 2));
			return (var.distance);
		}
...
void	init_horizontal_ray_steps(t_data *data, t_ray *ray, t_var *var)
{
	...
	if (ray->lookingdown)
		var->yintercept += T_S;
	var->xintercept = data->p_x + (var->yintercept - data->p_y)
		/ tan(ray->rayangle);
	var->ystep = T_S;

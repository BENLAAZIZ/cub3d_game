...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/30 00:33:19 by hben-laz          #+#    #+#             */
/*   Updated: 2024/12/01 18:03:18 by hben-laz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	drawplayer(t_data *data)
{
	...
	while (y < 3)
	{
		...
		while (x < 3)
		{
			mlx_put_pixel(data->img, player.ray_x * 10.0 + x,
				player.ray_y * 10.0 + y, ft_pixel(255, 0, 0, 255));
			x++;
		}
		y++;
	}
...
void	draw_rays_minimap(t_data *data, t_player *player)
{
	...
	while (rayangle < data->angle + (data->fov / 2))
	{
		while (player->z)
		{
			if (is_wall_min(data, player->ray_y, player->ray_x))
				break ;
			mlx_put_pixel(data->img, player->ray_x * 10.0 + 1,
				player->ray_y * 10.0 + 1, ft_pixel(255, 255, 0, 255));
...
void	map_suit_if(t_data *data, int i, int j)
{
	...
	if (data->all_map[i][j] == '1')
	{
		...
		while (y < 10)
		{
			...
			while (x < 10)
			{
				mlx_put_pixel(data->img, (j * 10) + y,
					(i * 10) + x, ft_pixel(255, 255, 255, 255));
				x++;
			}
			y++;
		}

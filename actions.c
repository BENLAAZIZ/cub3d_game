...
/*   Updated: 2024/11/20 21:30:06 by hben-laz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player_down(t_data *data)
{
	...
	x = data->p_x - cos(data->angle) * 15.00;
	y = data->p_y - sin(data->angle) * 15.00;
...
{
	double y = data->p_y + sin(data->angle) * 15.00;
	double x = data->p_x + cos(data->angle) * 15.00;
	if(data->all_map[(int)(y / TILE_SIZE)][(int)x / (int)TILE_SIZE] == '1')
		return ;
	data->p_y = y;
	data->p_x = x;
...
{ 
	double y = data->p_y + sin(data->angle - M_PI_2) * 15.00;
	double x = data->p_x + cos(data->angle - M_PI_2) * 15.00;
...
void	player_rot(t_data *data, int keycode)
{
	if (keycode == 123)
	{
		data->angle -= 0.1 * (M_PI / 180) * 100.00;
  		if (data->angle < 0)
			data->angle += 2 * M_PI;
